...
@dp.callback_query(F.data == "admin_list_delete_service")
async def list_delete_services(callback: types.CallbackQuery):
    services = db_query("SELECT id, name, country FROM services", fetchall=True)
    if not services:
        return await callback.message.edit_text("❌ No services to delete.", reply_markup=admin_main_menu())
    
    kb = InlineKeyboardBuilder()
    for s_id, name, country in services:
        kb.row(types.InlineKeyboardButton(text=f"🗑️ {name} ({country})", callback_data=f"admin_del_ser_{s_id}"))
    kb.row(types.InlineKeyboardButton(text="⬅️ Back", callback_data="admin_manage_services"))
    await callback.message.edit_text("🗑️ <b>Select a service to delete:</b>", reply_markup=kb.as_markup(), parse_mode="HTML")

@dp.callback_query(F.data.startswith("admin_del_ser_"))
async def delete_service_confirm(callback: types.CallbackQuery):
    ...
    await list_delete_services(callback)
...
async def check_balance(callback: types.CallbackQuery):
    ...
    if res and res.get('success'):
        ...
        await callback.message.edit_text(f"💰 <b>Your Nexa Balance:</b> {balance} ৳", reply_markup=admin_main_menu(), parse_mode="HTML")
    else:
        await callback.answer("❌ Failed to fetch balance.")
